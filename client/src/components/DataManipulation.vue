...
const emit = defineEmits(['update', 'delete', 'deselect', 'rename-column', 'assign-shortcut', 'send-row'])
...
const editingImageCol = ref('')

function openImageEditor(col) {
  editingImageCol.value = col
  editingImageSrc.value = cellValues.value[col]
  showImageEditor.value = true
}

function handleSaveEditedImage(dataUrl) {
  if (editingImageCol.value) {
    cellValues.value[editingImageCol.value] = dataUrl
    handleUpdate()
  }
  ...
  <div class="glass-panel-sm overflow-visible transition-all duration-300 relative z-50" :class="selectedRow ? 'opacity-100' : 'max-h-14 opacity-70'">
    ...
    <div v-if="selectedRow" class="p-5 animate-slide-down">
        ...
        >
          ...
          <div v-else-if="columnTypes[col] === 'image'" class="flex flex-col gap-2">
            <div v-if="cellValues[col]" class="relative w-full h-24 rounded-lg overflow-hidden border border-surface-700/60 group bg-surface-950/40">
              ...
              <div class="absolute inset-0 bg-surface-950/70 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-opacity duration-200 gap-2">
                <button 
                  @click="openImageEditor(col)" 
                  class="flex items-center justify-center text-accent-400 hover:text-accent-300 font-semibold text-xs gap-1"
                >
                ...
                <button 
                  ...
                  class="flex items-center justify-center text-danger-400 hover:text-danger-300 font-semibold text-xs gap-1"
                >
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div class="flex items-center gap-3 mt-4 pt-4 border-t border-surface-700/30">
        ...
        <button
          id="btn-send-row"
          class="btn-ghost border border-accent-500/30 hover:border-accent-500/50 hover:bg-accent-500/5 text-accent-400 text-xs py-2"
          @click="emit('send-row', props.selectedRow)"
        >
          <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
